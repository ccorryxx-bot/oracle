    ...
    <section class="relative py-20 px-4 text-center overflow-hidden">
      ...
      <div class="relative max-w-3xl mx-auto">
        ...
        <div class="inline-flex items-center gap-2 mb-6 px-3 py-1.5 rounded-full text-xs font-medium border"
          ...
          Myanmar #1 Game Website Marketplace
        </div>

        <h1 class="font-display font-bold text-white leading-tight mb-4"
          style="font-size:clamp(2rem,5vw,3.25rem)">
          Premium Game Websites<br />
          <span style="color:#A78BFA">ဝယ်ရောင်း</span> ပါ
        </h1>
        ...
        <div class="flex items-center gap-2 max-w-md mx-auto">
          <div class="flex-1 relative">
            ...
            <input v-model="search" type="text" placeholder="Website name ရှာပါ..."
          ...
          <button class="shrink-0 px-5 py-3 rounded-xl text-sm font-medium text-white transition-colors"
    ...
    <section class="max-w-7xl mx-auto px-4 pb-20">
      ...
      <div class="flex items-center justify-between mb-6">
        <p class="text-sm" style="color:#475569">
          <span class="text-white font-medium">{{ filteredListings.length }}</span> listings
        </p>
      ...
      <div v-else-if="filteredListings.length === 0" class="text-center py-24">
        ...
        <p class="font-display font-semibold text-white mb-2">Listing မရှိသေးပါ</p>
        <p class="text-sm mb-6" style="color:#475569">ပထမဆုံး Listing တင်လိုက်ပါ</p>
        <NuxtLink to="/sell"
          class="inline-flex text-sm font-medium text-white px-5 py-2.5 rounded-xl transition-colors"
          style="background:#7C3AED">+ Website ရောင်းမည်</NuxtLink>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <NuxtLink v-for="item in filteredListings" :key="item.id"
          ...
          <!-- Info -->
          <div class="p-5">
            <h3 class="font-display font-semibold text-white text-base leading-snug mb-1.5 line-clamp-1 transition-colors group-hover:text-violet-400">
              {{ item.title }}
            </h3>
            <p v-if="item.description"
              class="text-sm line-clamp-2 mb-4" style="color:#475569">
              {{ item.description }}
            </p>
...
const stats = [
  ...
  { label: 'Active Sellers',  value: '0+' },
  { label: 'Completed Deals', value: '0+' },
]
...
// ── Listings ──────────────────────────────────────
const { data: rawListings, pending } = await useAsyncData(
  'listings',
  async () => {
...
function formatMMK(price: number) {
  if (price >= 1_000_000)
    return (price / 1_000_000).toFixed(1) + 'M ks'
  if (price >= 1_000)
    return (price / 1_000).toFixed(0) + 'K ks'
  return price + ' ks'
}
</script>
